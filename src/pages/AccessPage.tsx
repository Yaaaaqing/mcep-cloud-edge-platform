...
export function AccessPage() {
  ...
  return (
    <div className="page-shell py-8 lg:py-10">
      ...
      <section className="mesh-bg card grid grid-cols-1 items-center gap-7 p-5 lg:grid-cols-[1.1fr_.9fr] lg:p-6 xl:gap-10 xl:p-8">
        <div><span className="inline-flex rounded-full bg-brand/10 px-3 py-1.5 text-xs font-semibold text-brand">五步式标准流程</span><h2 className="mt-5 text-2xl font-semibold leading-tight text-ink xl:text-3xl">从信息确认到测试上线，形成完整接入档案</h2><p className="mt-4 max-w-2xl leading-7 text-muted">表单支持自动保存，完成后可生成 JSON 或 Markdown 清单，并自动进入管理端的接入申请台账。</p><Button className="mt-6" variant="secondary" onClick={() => document.getElementById('standards')?.scrollIntoView()}>查看接入说明<ArrowRight size={17} /></Button></div>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4">{[['7', '标准模板'], ['5', '接入步骤'], ['3', '责任主体'], ['2', '导出格式']].map(([value, label]) => <div className="rounded-2xl border border-white bg-white/80 p-4 shadow-sm sm:p-5" key={label}><p className="text-3xl font-semibold text-brand">{value}</p><p className="mt-1 text-sm text-muted">{label}</p></div>)}</div>
      </section>
      <section className="mt-14" id="standards"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="section-kicker">模板入口</p><h2 className="section-title">接入材料与实施记录</h2></div><p className="text-sm text-muted">点击模板可下载 Markdown 示例</p></div><div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4">{templateItems.map((item, index) => <button key={item} onClick={() => downloadFile(`${item}.md`, `# ${item}\n\n> MCEP 接入模板\n\n请按平台接入规范填写并提交。\n`, 'text/markdown;charset=utf-8')} className="card group flex items-center gap-4 p-5 text-left transition hover:border-brand/30"><span className={`grid h-11 w-11 shrink-0 place-items-center rounded-xl ${index % 2 ? 'bg-cyan-50 text-cyan-600' : 'bg-blue-50 text-brand'}`}><FileText size={21} /></span><span className="text-sm font-medium leading-6 text-ink">{item}</span><ArrowRight size={16} className="ml-auto text-line transition group-hover:translate-x-1 group-hover:text-brand" /></button>)}</div></section>
