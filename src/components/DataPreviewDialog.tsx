...
export function DataPreviewDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  return <Modal open={open} onClose={onClose} title="CSV 数据预览" description="主轴温升试验数据_20260715.csv" width="max-w-6xl">
    ...
    <div className="mt-6 flex flex-col items-stretch justify-between gap-4 lg:flex-row lg:items-center">
      <div className="grid grid-cols-1 items-center gap-3 sm:grid-cols-[auto_1fr_auto_1fr]">
        <label className="text-sm text-muted">参数选择</label>
        <select className="input w-full" defaultValue="temperature"><option value="temperature">温度（℃）</option><option value="vibration">振动速度（mm/s）</option><option value="load">负载率（%）</option></select>
        <label className="text-sm text-muted">横轴</label>
        <select className="input w-full"><option>采集时间</option></select>
      </div>
      <Button icon={<Download size={16} />} onClick={() => downloadFile('主轴温升试验数据.csv', csvText, 'text/csv;charset=utf-8')}>下载 CSV</Button>
    </div>
    <div className="mt-5 h-64 min-w-0 rounded-2xl border border-line bg-white p-2 sm:h-72 sm:p-4">
    ...
    <div className="mt-5 overflow-hidden rounded-2xl border border-line">
      ...
      <div className="thin-scrollbar max-h-64 overflow-auto"><table className="min-w-[720px] w-full"><thead className="table-head sticky top-0"><tr>{['采集时间', '主轴转速(r/min)', '振动速度(mm/s)', '温度(℃)', '负载率(%)'].map((item) => <th className="px-4 py-3" key={item}>{item}</th>)}</tr></thead><tbody>{csvPreviewData.map((row) => <tr key={row.timestamp}><td className="table-cell">{row.timestamp}</td><td className="table-cell">{row.spindleSpeed}</td><td className="table-cell">{row.vibration}</td><td className="table-cell">{row.temperature}</td><td className="table-cell">{row.load}</td></tr>)}</tbody></table></div>
    </div>
  </Modal>
}
